...
{
	public partial class BlobFileTypesCMController : CMBaseApiControllerAuthorized
	{
			private const string GET_LIST_ROUTE_NAME = "BlobFileTypesCMList";
			private const int maxPageSize = 100;

			private GenericFactory<entCM.BlobFileType, dtoCM.BlobFileType> _factory 
				= new GenericFactory<entCM.BlobFileType, dtoCM.BlobFileType>();

		public BlobFileTypesCMController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "BlobFileTypes/{blobFileTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int blobFileTypeId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_BlobFileTypeAsync(blobFileTypeId);
		...
		[HttpGet]
		[VersionedRoute(template: "BlobFileTypes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_BlobFileType().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entCM.BlobFileType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "BlobFileTypes/{blobFileTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int blobFileTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BlobFileTypeAsync(blobFileTypeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, blobFileTypeId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "BlobFileTypes/{blobFileTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int blobFileTypeId, [FromBody] JsonPatchDocument<dtoCM.BlobFileType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_BlobFileTypeAsync(blobFileTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
				...
				dtoItem.BlobFileTypeId = blobFileTypeId;
		...
		[HttpPost]
		[VersionedRoute(template: "BlobFileTypes", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoCM.BlobFileType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.BlobFileTypeId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "BlobFileTypes/{blobFileTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int blobFileTypeId, [FromBody] dtoCM.BlobFileType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var updatedDBItem = _factory.Create(dtoItem); // map
				RunCustomLogicBeforeUpdatePut(ref updatedDBItem, blobFileTypeId);
				var result = await Repo.UpdateAsync(updatedDBItem);
		...
		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entCM.BlobFileType> dbItems, ref List<string> filterList);
	}
}