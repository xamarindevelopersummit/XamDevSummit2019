...
{
	public partial class UserProfilesCMController : CMBaseApiControllerAuthorized
	{
			private const string GET_LIST_ROUTE_NAME = "UserProfilesCMList";
			private const int maxPageSize = 100;

			private GenericFactory<entCM.UserProfile, dtoCM.UserProfile> _factory 
				= new GenericFactory<entCM.UserProfile, dtoCM.UserProfile>();

		public UserProfilesCMController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "UserProfiles/{userProfileId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int userProfileId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_UserProfileAsync(userProfileId);
		...
		[HttpGet]
		[VersionedRoute(template: "UserProfiles", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_UserProfile().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entCM.UserProfile).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "UserProfiles/{userProfileId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int userProfileId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_UserProfileAsync(userProfileId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, userProfileId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "UserProfiles/{userProfileId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int userProfileId, [FromBody] JsonPatchDocument<dtoCM.UserProfile> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_UserProfileAsync(userProfileId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "UserProfiles", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoCM.UserProfile dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.UserProfileId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "UserProfiles/{userProfileId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int userProfileId, [FromBody] dtoCM.UserProfile dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entCM.UserProfile> dbItems, ref List<string> filterList);
	}
}