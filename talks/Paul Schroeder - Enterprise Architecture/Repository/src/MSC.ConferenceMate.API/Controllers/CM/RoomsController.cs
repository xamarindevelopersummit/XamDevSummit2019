...
{
	public partial class RoomsCMController : CMBaseApiControllerAuthorized
	{
			private const string GET_LIST_ROUTE_NAME = "RoomsCMList";
			private const int maxPageSize = 100;

			private GenericFactory<entCM.Room, dtoCM.Room> _factory 
				= new GenericFactory<entCM.Room, dtoCM.Room>();

		public RoomsCMController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "Rooms/{roomId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int roomId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_RoomAsync(roomId);
		...
		[HttpGet]
		[VersionedRoute(template: "Rooms", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_Room().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entCM.Room).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Rooms/{roomId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int roomId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_RoomAsync(roomId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, roomId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "Rooms/{roomId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int roomId, [FromBody] JsonPatchDocument<dtoCM.Room> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_RoomAsync(roomId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Rooms", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoCM.Room dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.RoomId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "Rooms/{roomId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int roomId, [FromBody] dtoCM.Room dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var updatedDBItem = _factory.Create(dtoItem); // map
				RunCustomLogicBeforeUpdatePut(ref updatedDBItem, roomId);
				var result = await Repo.UpdateAsync(updatedDBItem);
		...
		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entCM.Room> dbItems, ref List<string> filterList);
	}
}