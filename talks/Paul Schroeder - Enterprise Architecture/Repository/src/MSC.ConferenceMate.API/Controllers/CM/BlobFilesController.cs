...
{
	public partial class BlobFilesCMController : CMBaseApiControllerAuthorized
	{
			private const string GET_LIST_ROUTE_NAME = "BlobFilesCMList";
			private const int maxPageSize = 100;

			private GenericFactory<entCM.BlobFile, dtoCM.BlobFile> _factory 
				= new GenericFactory<entCM.BlobFile, dtoCM.BlobFile>();

		public BlobFilesCMController() : base()
		{
		}

		public BlobFilesCMController(ILoggingService log, ICMRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "BlobFiles/{blobFileId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(System.Guid blobFileId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_BlobFileAsync(blobFileId);
		...
		[HttpGet]
		[VersionedRoute(template: "BlobFiles", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_BlobFile().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entCM.BlobFile).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "BlobFiles/{blobFileId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(System.Guid blobFileId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_BlobFileAsync(blobFileId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, blobFileId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "BlobFiles/{blobFileId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid blobFileId, [FromBody] JsonPatchDocument<dtoCM.BlobFile> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_BlobFileAsync(blobFileId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "BlobFiles", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoCM.BlobFile dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.BlobFileId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "BlobFiles/{blobFileId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid blobFileId, [FromBody] dtoCM.BlobFile dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entCM.BlobFile> dbItems, ref List<string> filterList);
	}
}