...
{
	public partial class LogsCMController : CMBaseApiControllerAuthorized
	{
			private const string GET_LIST_ROUTE_NAME = "LogsCMList";
			private const int maxPageSize = 100;

			private GenericFactory<entCM.Log, dtoCM.Log> _factory 
				= new GenericFactory<entCM.Log, dtoCM.Log>();

		public LogsCMController() : base()
		{
		}

		public LogsCMController(ILoggingService log, ICMRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "Logs/{id}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int id)
		{
			try
			{
				...
				var result = await Repo.Delete_LogAsync(id);
		...
		[HttpGet]
		[VersionedRoute(template: "Logs", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_Log().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entCM.Log).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "Logs/{id}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int id, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_LogAsync(id, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "Logs/{id}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int id, [FromBody] JsonPatchDocument<dtoCM.Log> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_LogAsync(id, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "Logs", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoCM.Log dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		[HttpPut]
		[VersionedRoute(template: "Logs/{id}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int id, [FromBody] dtoCM.Log dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entCM.Log> dbItems, ref List<string> filterList);
	}
}