...
{
	public partial class FeedbackInitiatorTypesCMController : CMBaseApiControllerAuthorized
	{
			private const string GET_LIST_ROUTE_NAME = "FeedbackInitiatorTypesCMList";
			private const int maxPageSize = 100;

			private GenericFactory<entCM.FeedbackInitiatorType, dtoCM.FeedbackInitiatorType> _factory 
				= new GenericFactory<entCM.FeedbackInitiatorType, dtoCM.FeedbackInitiatorType>();

		public FeedbackInitiatorTypesCMController() : base()
		{
		}
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				dbItems = dbItems.ApplySort(sort ?? (typeof(entCM.FeedbackInitiatorType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpPatch]
		[VersionedRoute(template: "FeedbackInitiatorTypes/{feedbackInitiatorTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int feedbackInitiatorTypeId, [FromBody] JsonPatchDocument<dtoCM.FeedbackInitiatorType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		public async Task<IHttpActionResult> Post([FromBody] dtoCM.FeedbackInitiatorType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		[HttpPut]
		[VersionedRoute(template: "FeedbackInitiatorTypes/{feedbackInitiatorTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int feedbackInitiatorTypeId, [FromBody] dtoCM.FeedbackInitiatorType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterInsert(ref entCM.FeedbackInitiatorType newDBItem, ref IRepositoryActionResult<entCM.FeedbackInitiatorType> result);
		...
		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entCM.FeedbackInitiatorType> dbItems, ref List<string> filterList);
	}
}