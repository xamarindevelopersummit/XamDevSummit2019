...
{
	public partial class FeedbackTypesCMController : CMBaseApiControllerAuthorized
	{
			private const string GET_LIST_ROUTE_NAME = "FeedbackTypesCMList";
			private const int maxPageSize = 100;

			private GenericFactory<entCM.FeedbackType, dtoCM.FeedbackType> _factory 
				= new GenericFactory<entCM.FeedbackType, dtoCM.FeedbackType>();

		public FeedbackTypesCMController() : base()
		{
		}

		public FeedbackTypesCMController(ILoggingService log, ICMRepository repository)
		: base(log, repository)
		{
		}
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				dbItems = dbItems.ApplySort(sort ?? (typeof(entCM.FeedbackType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpPatch]
		[VersionedRoute(template: "FeedbackTypes/{feedbackTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int feedbackTypeId, [FromBody] JsonPatchDocument<dtoCM.FeedbackType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		public async Task<IHttpActionResult> Post([FromBody] dtoCM.FeedbackType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		[HttpPut]
		[VersionedRoute(template: "FeedbackTypes/{feedbackTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int feedbackTypeId, [FromBody] dtoCM.FeedbackType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var updatedDBItem = _factory.Create(dtoItem); // map
				RunCustomLogicBeforeUpdatePut(ref updatedDBItem, feedbackTypeId);
				var result = await Repo.UpdateAsync(updatedDBItem);
		...
		partial void RunCustomLogicAfterUpdatePatch(ref entCM.FeedbackType updatedDBItem, ref IRepositoryActionResult<entCM.FeedbackType> result);
		...
		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entCM.FeedbackType> dbItems, ref List<string> filterList);
	}
}