	...
	public partial class FeaturedEventsCMController : CMBaseApiControllerAuthorized
	{
		...
		partial void RunCustomLogicAfterUpdatePut(ref entCM.FeaturedEvent updatedDBItem, ref IRepositoryActionResult<entCM.FeaturedEvent> result)
		{
			...
			{	// An update/PUT was attempted when it should have been a create/POST.
				...
				var insertResult = Utils.AsyncHelper.RunSync<IRepositoryActionResult<entCM.FeaturedEvent>>(() => Repo.InsertAsync(localDBItem));
				if (insertResult.Status == Enums.RepositoryActionStatus.Created)
				{   // Insert worked
					result = new RepositoryActionResult<entCM.FeaturedEvent>(insertResult.Entity, Enums.RepositoryActionStatus.Updated);
				}
			}
		}
		...
		// {
		// 	var existingDBItem = Utils.AsyncHelper.RunSync<entCM.FeaturedEvent>(() => Repo.Get_FeaturedEventAsync(featuredEventId, 1));
