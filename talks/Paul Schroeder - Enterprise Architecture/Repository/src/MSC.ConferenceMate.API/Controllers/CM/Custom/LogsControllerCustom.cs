	...
	public partial class LogsCMController : CMBaseApiControllerAuthorized
	{
		...
		partial void RunCustomLogicAfterUpdatePut(ref entCM.Log updatedDBItem, ref IRepositoryActionResult<entCM.Log> result)
		{
			...
			{	// An update/PUT was attempted when it should have been a create/POST.
				...
				var insertResult = Utils.AsyncHelper.RunSync<IRepositoryActionResult<entCM.Log>>(() => Repo.InsertAsync(localDBItem));
				if (insertResult.Status == Enums.RepositoryActionStatus.Created)
				{   // Insert worked
					result = new RepositoryActionResult<entCM.Log>(insertResult.Entity, Enums.RepositoryActionStatus.Updated);
				}
			}
		}
		...
		// {
		// 	var existingDBItem = Utils.AsyncHelper.RunSync<entCM.Log>(() => Repo.Get_LogAsync(id, 1));
