	...
	public partial class AnnouncementsCMController : CMBaseApiControllerAuthorized
	{
		...
		partial void RunCustomLogicAfterUpdatePut(ref entCM.Announcement updatedDBItem, ref IRepositoryActionResult<entCM.Announcement> result)
		{
			...
			{	// An update/PUT was attempted when it should have been a create/POST.
				...
				var insertResult = Utils.AsyncHelper.RunSync<IRepositoryActionResult<entCM.Announcement>>(() => Repo.InsertAsync(localDBItem));
				if (insertResult.Status == Enums.RepositoryActionStatus.Created)
				{   // Insert worked
					result = new RepositoryActionResult<entCM.Announcement>(insertResult.Entity, Enums.RepositoryActionStatus.Updated);
				}
			}
		}
		...
		// {
		// 	var existingDBItem = Utils.AsyncHelper.RunSync<entCM.Announcement>(() => Repo.Get_AnnouncementAsync(announcementId, 1));
