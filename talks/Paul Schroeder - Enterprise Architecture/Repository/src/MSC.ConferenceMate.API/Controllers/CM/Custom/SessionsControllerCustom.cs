	...
	public partial class SessionsCMController : CMBaseApiControllerAuthorized
	{
		...
		partial void RunCustomLogicAfterUpdatePut(ref entCM.Session updatedDBItem, ref IRepositoryActionResult<entCM.Session> result)
		{
			...
			{	// An update/PUT was attempted when it should have been a create/POST.
				...
				var insertResult = Utils.AsyncHelper.RunSync<IRepositoryActionResult<entCM.Session>>(() => Repo.InsertAsync(localDBItem));
				if (insertResult.Status == Enums.RepositoryActionStatus.Created)
				{   // Insert worked
					result = new RepositoryActionResult<entCM.Session>(insertResult.Entity, Enums.RepositoryActionStatus.Updated);
				}
			}
		}
		...
		// {
		// 	var existingDBItem = Utils.AsyncHelper.RunSync<entCM.Session>(() => Repo.Get_SessionAsync(sessionId, 1));
