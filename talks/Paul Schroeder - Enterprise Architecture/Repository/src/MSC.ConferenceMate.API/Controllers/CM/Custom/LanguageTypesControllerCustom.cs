	...
	public partial class LanguageTypesCMController : CMBaseApiControllerAuthorized
	{
		...
		partial void RunCustomLogicAfterUpdatePut(ref entCM.LanguageType updatedDBItem, ref IRepositoryActionResult<entCM.LanguageType> result)
		{
			...
			{	// An update/PUT was attempted when it should have been a create/POST.
				...
				var insertResult = Utils.AsyncHelper.RunSync<IRepositoryActionResult<entCM.LanguageType>>(() => Repo.InsertAsync(localDBItem));
				if (insertResult.Status == Enums.RepositoryActionStatus.Created)
				{   // Insert worked
					result = new RepositoryActionResult<entCM.LanguageType>(insertResult.Entity, Enums.RepositoryActionStatus.Updated);
				}
			}
		}
		...
		// {
		// 	var existingDBItem = Utils.AsyncHelper.RunSync<entCM.LanguageType>(() => Repo.Get_LanguageTypeAsync(languageTypeId, 1));
