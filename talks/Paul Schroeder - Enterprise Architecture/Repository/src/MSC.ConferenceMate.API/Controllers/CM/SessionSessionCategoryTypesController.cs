...
{
	public partial class SessionSessionCategoryTypesCMController : CMBaseApiControllerAuthorized
	{
			private const string GET_LIST_ROUTE_NAME = "SessionSessionCategoryTypesCMList";
			private const int maxPageSize = 100;

			private GenericFactory<entCM.SessionSessionCategoryType, dtoCM.SessionSessionCategoryType> _factory 
				= new GenericFactory<entCM.SessionSessionCategoryType, dtoCM.SessionSessionCategoryType>();

		public SessionSessionCategoryTypesCMController() : base()
		{
		}
		...
		[HttpDelete]
		[VersionedRoute(template: "SessionSessionCategoryTypes/{sessionId}/{sessionCategoryTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int sessionId, int sessionCategoryTypeId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_SessionSessionCategoryTypeAsync(sessionId, sessionCategoryTypeId);
		...
		[HttpGet]
		[VersionedRoute(template: "SessionSessionCategoryTypes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_SessionSessionCategoryType().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entCM.SessionSessionCategoryType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "SessionSessionCategoryTypes/{sessionId}/{sessionCategoryTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int sessionId, int sessionCategoryTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_SessionSessionCategoryTypeAsync(sessionId, sessionCategoryTypeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, sessionId, sessionCategoryTypeId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "SessionSessionCategoryTypes/{sessionId}/{sessionCategoryTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int sessionId, int sessionCategoryTypeId, [FromBody] JsonPatchDocument<dtoCM.SessionSessionCategoryType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_SessionSessionCategoryTypeAsync(sessionId, sessionCategoryTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "SessionSessionCategoryTypes", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoCM.SessionSessionCategoryType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.SessionId}/{newDTOItem.SessionCategoryTypeId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "SessionSessionCategoryTypes/{sessionId}/{sessionCategoryTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int sessionId, int sessionCategoryTypeId, [FromBody] dtoCM.SessionSessionCategoryType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		partial void RunCustomLogicAfterGetQueryableList(ref IQueryable<entCM.SessionSessionCategoryType> dbItems, ref List<string> filterList);
	}
}