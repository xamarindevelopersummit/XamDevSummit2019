...
namespace MSC.ConferenceMate.API.Controllers.CM
{
	public partial class FeedbacksCMController : CMBaseApiControllerAuthorized
	{
			...
			private GenericFactory<entCM.Feedback, dtoCM.Feedback> _factory 
				= new GenericFactory<entCM.Feedback, dtoCM.Feedback>();

		public FeedbacksCMController() : base()
		{
		}

		public FeedbacksCMController(ILoggingService log, ICMRepository repository)
		: base(log, repository)
		{
		}
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				dbItems = dbItems.ApplySort(sort ?? (typeof(entCM.Feedback).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpPatch]
		[VersionedRoute(template: "Feedbacks/{feedbackId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(System.Guid feedbackId, [FromBody] JsonPatchDocument<dtoCM.Feedback> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		public async Task<IHttpActionResult> Post([FromBody] dtoCM.Feedback dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
		...
		[HttpPut]
		[VersionedRoute(template: "Feedbacks/{feedbackId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(System.Guid feedbackId, [FromBody] dtoCM.Feedback dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var updatedDBItem = _factory.Create(dtoItem); // map
				RunCustomLogicBeforeUpdatePut(ref updatedDBItem, feedbackId);
				var result = await Repo.UpdateAsync(updatedDBItem);
