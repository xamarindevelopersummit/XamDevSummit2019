...
namespace MSC.ConferenceMate.API.Controllers.CM
{
	...
	{
			private const string GET_LIST_ROUTE_NAME = "SessionCategoryTypesCMList";
			private const int maxPageSize = 100;

			private GenericFactory<entCM.SessionCategoryType, dtoCM.SessionCategoryType> _factory 
				= new GenericFactory<entCM.SessionCategoryType, dtoCM.SessionCategoryType>();

		public SessionCategoryTypesCMController() : base()
		{
		}

		public SessionCategoryTypesCMController(ILoggingService log, ICMRepository repository)
		: base(log, repository)
		{
		}

		[HttpDelete]
		[VersionedRoute(template: "SessionCategoryTypes/{sessionCategoryTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Delete(int sessionCategoryTypeId)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }

				var result = await Repo.Delete_SessionCategoryTypeAsync(sessionCategoryTypeId);
		...
		[HttpGet]
		[VersionedRoute(template: "SessionCategoryTypes", allowedVersion: 1, Name = GET_LIST_ROUTE_NAME)]
		...
		string fields = null, string filter = null, int page = 1, int pageSize = maxPageSize)
		{
			try
			{
				...
				var dbItems = Repo.GetQueryable_SessionCategoryType().AsNoTracking();
				RunCustomLogicAfterGetQueryableList(ref dbItems, ref filterList);
				dbItems = dbItems.ApplyFilter(filterList);
				dbItems = dbItems.ApplySort(sort ?? (typeof(entCM.SessionCategoryType).GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)).First().Name);
		...
		[HttpGet]
		[VersionedRoute(template: "SessionCategoryTypes/{sessionCategoryTypeId}/{numChildLevels:int=0}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Get(int sessionCategoryTypeId, int numChildLevels)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				var dbItem = await Repo.Get_SessionCategoryTypeAsync(sessionCategoryTypeId, numChildLevels);
				...
				RunCustomLogicOnGetEntityByPK(ref dbItem, sessionCategoryTypeId, numChildLevels);
		...
		[HttpPatch]
		[VersionedRoute(template: "SessionCategoryTypes/{sessionCategoryTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Patch(int sessionCategoryTypeId, [FromBody] JsonPatchDocument<dtoCM.SessionCategoryType> patchDocument)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var dbItem = await Repo.Get_SessionCategoryTypeAsync(sessionCategoryTypeId, numChildLevels: 0);
				if (dbItem == null)
				{
					return NotFound();
				}
		...
		[HttpPost]
		[VersionedRoute(template: "SessionCategoryTypes", allowedVersion: 1)]
		public async Task<IHttpActionResult> Post([FromBody] dtoCM.SessionCategoryType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				{   // map to dto
					...
					return Created($"{uriFormatted}/{newDTOItem.SessionCategoryTypeId}", newDTOItem);
				}
		...
		[HttpPut]
		[VersionedRoute(template: "SessionCategoryTypes/{sessionCategoryTypeId}", allowedVersion: 1)]
		public async Task<IHttpActionResult> Put(int sessionCategoryTypeId, [FromBody] dtoCM.SessionCategoryType dtoItem)
		{
			try
			{
				if (!base.OnActionExecuting(out HttpStatusCode httpStatusCode, out string message)) { return Content(httpStatusCode, message); }
				...
				var updatedDBItem = _factory.Create(dtoItem); // map
				RunCustomLogicBeforeUpdatePut(ref updatedDBItem, sessionCategoryTypeId);
				var result = await Repo.UpdateAsync(updatedDBItem);
